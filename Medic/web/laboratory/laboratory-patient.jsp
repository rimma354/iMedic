...
<html>
    ...
    <body>
        ...
        <%
            ...
            String pResults = request.getParameter("results");
            String pAction = request.getParameter("action");
            String pIdReception = request.getParameter("idReception");
            ...
            LaboratoryReception ckechedReception;
        ...
        <%
            ...
            if (card != null) {
                 if ("newResult".equals(pAction)) {
                    if (pResults.trim().isEmpty()) {
        %>
        <div class="alert alert-error">
            <button type="button" class="close" data-dismiss="alert">&times;</button>
            <h4>Error!</h4>
            You couldn't add empty results.
        </div>
        <%    } else {
                        idRec = Integer.valueOf(pIdReception);
                        ckechedReception = localLabRec.find(idRec);
                        ckechedReception.setAnalysisResult(pResults);
                        localLabRec.edit(ckechedReception); %>
        <div class="alert alert-success">
            <button type="button" class="close" data-dismiss="alert">&times;</button>
            You successfully added new results.
        </div>
        <%
                    }
                }
        ...
        <div class="tabbable">
            <ul class="nav nav-tabs" >
                <li <% if (pAction == null) {%>class="active"<%}%>><a href="#tab1" data-toggle="tab" >Personal data</a></li>
                <li><a href="#tab2"data-toggle="tab" >Additional information</a></li>
                <li<% if ("newResult".equals(pAction)) {%> class="active" <%}%>><a href="#tab3"data-toggle="tab">Analysis</a></li>
            </ul>
            <div class="tab-content">
                <div class="tab-pane <% if (pAction == null){%>active<%}%>" id="tab1">
                    <div class="container" >
                ...
                <div class="tab-pane <% if ("newResult".equals(pAction)) {%>active<%}%>" id="tab3">
                    <div class="container" >
                        <table  class="table table-hover">
                            <thead>
                                <tr>
                            ...
                            <div class="modal" id="addResult<%=labRecId%>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
                                <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                                    <h5 id="myModalLabel">Adding results of "<%=analysisTitle%>", by <%=analysisDateStr%></h5>
                                </div>
                                <div>
                                    <form class="form-horizontal" method="POST">
                                        <input type="hidden" name="idReception" value="<%=labRecId%>">
                                        <div class="modal-body">
                                            <div class="control-group ">
                                                <label class="my-control-label" for="outputResults">Results</label>
                                                <div>
                                                    <textarea rows="9" id="outputResults" name="results" required style="width:  510px;"></textarea>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button class="btn" data-dismiss="modal" aria-hidden="true" formnovalidate>Close</button>
                                            <button type="submit" name="action" value="newResult" class="btn btn-primary">Save</button>
                                        </div>
                            ...
                            <tr>
                                ...
                                <td <% if (analysisResult == null) { %> class="text-warning"<%}%> ><% if (analysisResult != null) {%>
                                    ...
                                    <% } else {
                                        ...
                                        if (laboratory != null && analysisDate != null) {%>
                                    <a href="#addResult<%=labRecId%>"  data-toggle="modal">add</a> <%}
                                        }%>
                                </td>
        ...
        <div class="alert alert-error">
            <button type="button" class="close" data-dismiss="alert">&times;</button>
            <h4>Error!</h4>
            Your should fill in field before searching! <a href="laboratory-search.jsp">Return back </a> and enter ID.
        </div>
        <%
                }
            }%>
